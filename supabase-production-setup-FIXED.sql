-- =====================================================
-- COMPLETE PRODUCTION-READY SUPABASE SETUP (FIXED)
-- =====================================================
-- FIXED: Removed problematic large text indexes
...
-- =====================================================
-- PART 4: PERFORMANCE INDEXES (FIXED - NO LARGE TEXT)
-- =====================================================

-- FIXED: Index only on agent_id (not on extracted_text which is too large)
CREATE INDEX IF NOT EXISTS idx_website_pages_agent
ON website_pages(agent_id);

-- FIXED: Index only on kb_id (not on content which is too large)
...
RAISE NOTICE '✅ Performance indexes created (fixed for large text columns)';
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added extracted_text_tsv column';
  ELSE
    RAISE NOTICE 'extracted_text_tsv column already exists';
  END IF;
END $$;

-- Create GIN index for full-text search (THIS is the correct way for text search)
...
BEGIN
  ...
  RAISE NOTICE '✅ Performance indexes created (FIXED for large text)';
  RAISE NOTICE '✅ Full-text search optimized (GIN index on tsvector)';
  RAISE NOTICE '✅ RLS policies strengthened';
  RAISE NOTICE '✅ Data integrity constraints added';
  RAISE NOTICE '';
