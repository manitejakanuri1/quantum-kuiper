/**
 * Multi-Agent RAG Testing - Test 3-5 agents to validate system-wide improvements
 */
...
// Universal test queries that should work for any business website
const universalTestQueries = [
    {
...
async function getActiveAgents(limit = 5) {
    ...
    const { data: agents, error } = await supabase
        ...
        `)
        .eq('knowledge_bases.status', 'ready')
        .limit(limit);
    ...
    // Get chunk counts for each knowledge base
    const validAgents = [];
    for (const agent of agents) {
        if (agent.knowledge_bases && agent.knowledge_bases.length > 0) {
            ...
            if (!countError && count > 0) {
                agent.knowledge_bases[0].total_chunks = count;
                validAgents.push(agent);
            }
        }
    }

    console.log(`✅ Found ${validAgents.length} agents with ready knowledge bases\n`);

    return validAgents.slice(0, limit);
}
...
async function testSingleAgent(agent, testQueries) {
    ...
    console.log('\n' + '─'.repeat(80));
    console.log(`📊 Agent Performance Summary: ${agentName}`);
    console.log('─'.repeat(80));
...
async function runMultiAgentTest() {
    ...
    console.log('Testing RAG improvements across multiple agents to validate system-wide performance\n');

    // Get 3-5 active agents
    const agents = await getActiveAgents(5);

    if (agents.length === 0) {
        console.error('❌ No agents with ready knowledge bases found');
        console.log('\n💡 Please ensure agents have been crawled and have knowledge bases ready');
        return;
    }

    console.log(`📋 Testing ${agents.length} agents with ${universalTestQueries.length} queries each\n`);

    const allAgentResults = [];

    // Test each agent
    for (const agent of agents) {
        const agentResult = await testSingleAgent(agent, universalTestQueries);
    ...
    console.log(`📊 Total Test Queries: ${allAgentResults.length * universalTestQueries.length}`);
    ...
    for (const result of allAgentResults) {
        ...
        console.log(`   Accuracy: ${result.metrics.accuracy.toFixed(1)}% | F1: ${result.metrics.f1Score.toFixed(1)}% | Chunks: ${result.totalChunks}`);
    ...
    const categories = [...new Set(universalTestQueries.map(q => q.category))];
