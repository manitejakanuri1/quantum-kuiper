...
let currentOpenAIStream: { stream: AsyncIterable<any>; controller: AbortController } | null = null;

// Connection manager to keep track of active connections
const connections = new Map();
app.post('/start-conversation', (req: Request, res: Response) => {
  const { prompt, voiceId } = req.body as { prompt: string; voiceId: string };
  if (!prompt || !voiceId) {
    res.status(400).json({ error: 'Prompt and voiceId are required' });
    return;
  }
  ...
  if (!validate.valid) {
    ...
    res.status(400).json({ error: 'API key invalid: ' + validate.errors });
    return;
  }
...
const setupWebSocket = (ws: WebSocket, initialPrompt: string, voiceId: string, connectionId: string | string[]) => {
  ...
  deepgram.addListener(LiveTranscriptionEvents.Error, (error: unknown) => {
...
async function promptLLM(ws: WebSocket, initialPrompt: string, prompt: string, voiceId: string, connectionId: string | string[]) {
  try {
    ...
    let elevenLabsWs: WebSocket | undefined = undefined;
    ...
    try {
      for await (const chunk of stream) {
        ...
        if (!elevenLabsWs && fullResponse.length > 0) {
          elevenLabsWs = await startElevenLabsStreaming(ws, voiceId, connectionId) as WebSocket;
        }
